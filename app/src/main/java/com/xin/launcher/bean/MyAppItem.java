package com.xin.launcher.bean;

import android.support.annotation.DrawableRes;

import com.xin.launcher.M.MyAppBox;
...
public class MyAppItem extends FavoritesItem {
    ...
    public MyAppItem matchName(String sName) {
        if (isFolder()) {
            for (MyAppItem appItem : subApps) {
                if (appItem.getName().equals(sName)) {
                    return appItem;
                }
            }
        } else {
            if (getName().equals(sName)) {
                return this;
            }
        }
        return null;
