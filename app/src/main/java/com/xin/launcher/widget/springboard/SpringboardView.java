...
//    #触摸相关
    ...
    enum MODE {
        ...
        FREE, DRAGGING, SCROLL, SCROLL_OUT_LEFT, SCROLL_OUT_RIGHT
    }
        ...
        switch (action) {
            case MotionEvent.ACTION_MOVE:
                switch (mode) {
                    ...
                    case SCROLL:
                        int deltaX = (int) (mLastMotionX - x);
                        mLastMotionX = x;
                        if (deltaX > 0 && getScrollX() >= (getTotalPage() - 1) * getWidth()) {// 超出右边界
                            getParent().requestDisallowInterceptTouchEvent(false);
                            // scrollBy(getScrollX()-(getTotalPage() - 1) * getWidth()   , 0); // 回到原来位置
                            mode = MODE.SCROLL_OUT_RIGHT;
                        } else if (deltaX < 0 && getScrollX() <= 0) {// 超出左边界
                            getParent().requestDisallowInterceptTouchEvent(false);
                            mode = MODE.SCROLL_OUT_LEFT;
                        } else {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (mode == MODE.DRAGGING) {
                    ...
                    endScroll(distance);
                } else if (mode == MODE.SCROLL_OUT_RIGHT) {
                    scrollBy((getTotalPage() - 1) * getWidth() - getScrollX(), 0); // 回到原来位置
                    getParent().requestDisallowInterceptTouchEvent(true);
                    mode = MODE.FREE;
                } else if (mode == MODE.SCROLL_OUT_LEFT) {
                    getParent().requestDisallowInterceptTouchEvent(true);
                    mode = MODE.FREE;
...
//        showDropAnimation(x, y);
    ...
    protected boolean ifCanScroll(int deltaX) {
        ...
        if (Math.abs(deltaX) > mTouchSlop) {
            if (deltaX < 0) {
              ...
              /*  if (deltaX + getScrollX() < getWidth() ) {
                    return false;
                }*/
                if (getScrollX() < (getTotalPage() - 1) * getWidth()) {
                    return true;
                }
    ...
    /*mLastMotionX + getScrollX() >= child.getLeft()
                        && mLastMotionX + getScrollX() <  getWidth()*/
...
//            Log.e("SpringboardView", "onItemLongClick 长按 位置是 " + index);
            if (ifCanMove(index)) {
                getAdapter().setEditing(true);// 可以移动的的项启动编辑状态
                startDrag(v);
    ...
    public void configView(final FrameLayout view) {
        ...
        FrameLayout.LayoutParams params = new FrameLayout.LayoutParams(ScreenUtils.dp2px(20),
                ScreenUtils.dp2px(20));
        params.gravity = Gravity.TOP | Gravity.RIGHT;
        params.setMargins(0, 4, 4, 0);
        imageView.setId(R.id.delete_button_id);
        ((FrameLayout) view.findViewById(R.id.center_zone_id)).addView(imageView, params);
        imageView.setVisibility(View.GONE);
