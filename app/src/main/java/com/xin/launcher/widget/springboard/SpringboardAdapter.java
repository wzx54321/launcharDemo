...
public abstract class SpringboardAdapter<T extends FavoritesItem> {
    ...
    public static boolean IS_DATACAHNGE;
...
//            Log.e("SpringboardAdapter", "编辑模式" + isEditing);
            ...
            if (folderView != null && (folder = folderView.get()) != null) {
                ...
                if (folder.getEditingChangeListener() != null) {
                    folder.getEditingChangeListener().onChange(isEditting());
                }
...
//        Log.e("SpringboardAdapter","onExchangeItem fromPosition = "+fromPosition+"  toPosition = "+toPosition);
        ...
        if (dataCahngeListener != null /*&& !isTouching*/) {
            dataCahngeListener.onExchangeItem(new ArrayList<>(items));
        }
    ...
    public void changeFolderName(String name) {
        ...
        if (item.isFolder()) {
            ...
            if (!oldName.equals(name)) {
                ...
                onDataChange();
                if (dataCahngeListener != null && !isTouching) {
                    dataCahngeListener.onChangeFolderName(app);
                }
...
//        Log.e("SpringboardAdapter","onMergeItem fromPosition = "+fromPosition+"  toPosition = "+toPosition);
        ...
        if (dataCahngeListener != null && !isTouching) {
            dataCahngeListener.onMergeItem(needCreateId, toItem, fromItem, items);
        }
...
//        Log.e("SpringboardAdapter","onExChangeSubItem folderPosition = "+folderPosition +"  fromPosition = "+fromPosition+"  toPosition = "+toPosition);
...
//        (T)folder.getMenuList().remove(fromPosition);
        ...
        if (dataCahngeListener != null) {
            dataCahngeListener.onExChangeSubItem(folder);
        }
...
//        Log.e("SpringboardAdapter","deleteItem folderPosition = "+folderPosition+"  position = "+position);
    ...
    public interface DataChangeListener<T> {
         ...
         * 交换位置
         */
        void onExchangeItem(ArrayList<T> ts);

        /**
         * 修改文件夹的名称
         */
        void onChangeFolderName(T app);
        ...
        void onMergeItem(boolean needCreateId, T toItem, T fromItem, ArrayList<T> items);

        void onExChangeSubItem(T folder);
