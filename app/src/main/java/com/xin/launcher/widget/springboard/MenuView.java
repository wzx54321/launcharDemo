...
public class MenuView extends SpringboardView {
    ...
    public void onItemClick(final int position) {
        if (getAdapter().getItem(position).isFolder()) {
            if (activity != null) {
                Bitmap bitmap = BitmapLruCacheUtil.getBitmapFromMemoryCache(getmCurScreen() + "screen_shot_blur");// 取缓存
                if (bitmap != null) {
                    showFolder(position, new BitmapDrawable(getResources(), bitmap));
                } else {
                    bitmap = BitmapLruCacheUtil.getBitmapFromMemoryCache(getmCurScreen() + "screen_shot");
                    if (bitmap == null) {
                        bitmap = ImageResizer.scaledBitmap(ScreenUtils.shotScreen(activity),
                                        ScreenUtils.getScreenInfo()[0] / 3,
                                        ScreenUtils.getScreenInfo()[1] / 3);
                        BitmapLruCacheUtil.addBitmapToMemoryCache(getmCurScreen() + "screen_shot", bitmap);
                    }
                    getFolderBg(bitmap).subscribe(new Consumer<Drawable>() {
                        @Override
                        public void accept(Drawable drawable) {
                            showFolder(position, drawable);
                        }
                    });
                }
            } else {
                showFolder(position, null);
            }
        } else {
    ...
    private Observable<Drawable> getFolderBg(final Bitmap bitmap) {
        ...
        return Observable.create(new ObservableOnSubscribe<Drawable>() {
            ...
            public void subscribe(ObservableEmitter<Drawable> emitter) {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
                    bitmap1 = BlurUtils.rsBlur(getContext(), bitmap, 25);
                } else {
                    bitmap1 = BlurUtils.blur(bitmap, 25);
                }
                BitmapLruCacheUtil.clearCacheByKey(getmCurScreen() + "screen_shot_blur");
                BitmapLruCacheUtil.addBitmapToMemoryCache(getmCurScreen() + "screen_shot_blur", bitmap1);
                emitter.onNext(new BitmapDrawable(activity.getResources(), bitmap1));
...
//            Log.e("dragOnChild","移除View区域,删除位置是"+temChangPosition + " 的按钮");
     ...
     * 显示文件夹
     */
    public void showFolder(int position, Drawable drawable) {
        if (getAdapter().isEditting()) {
            stopShake();
            isStopShake = true;
        }
        ...
        if (drawable != null) {
            ...
            RelativeLayout folderRoot = folderView.findViewById(R.id.root_folder);
            folderRoot.setBackground(drawable);
        }
        LinearLayout linearLayout = folderView.findViewById(R.id.folder_container);

        if (folderViewBackground != null) {
            linearLayout.setBackground(folderViewBackground);
        }
        final EditText editText = folderView.findViewById(R.id.editText1);
        if (getAdapter().isEditting()) {
            editText.setBackgroundResource(R.drawable.folder_background);
       ...
       /* if (editTextBackground != null) {
        ...
        layout.setEditingChangeListener(new FolderView.onEditingChangeListener() {
            @Override
            public void onChange(boolean isEditing) {
                if (isEditing) {
                    editText.setBackgroundResource(R.drawable.folder_background);
                    editText.setEnabled(true);
                } else {
        ...
        SysUtils.hideBottomUIMenu(folderView);
