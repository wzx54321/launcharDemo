...
public class ModelHome {
    ...
    public String parserAudioResult(RecognizerResult results) {
    ...
    public void deleteItem(ArrayList<MyAppItem> myAppItems, MyAppItem app) {
        app.setDisplay(false);
        app.setSort(Integer.MAX_VALUE);// 不显示的设置为最大值了

        getAppBox().update(app);

        for (MyAppItem it : myAppItems) {
            it.setSort(myAppItems.indexOf(it));// 设置sort
        }

        getAppBox().update(myAppItems);
    }

    public void exchangeItem(ArrayList<MyAppItem> myAppItems) {
        for (MyAppItem it : myAppItems) {
            it.setSort(myAppItems.indexOf(it));// 设置sort
    ...
    public void changeFolderName(MyAppItem app) {
        getAppBox().update(app);
    }

    public void exChangeSubItem(MyAppItem folder) {
        if (folder.isFolder()) {
            ...
            getAppBox().update(items);
        }
        getAppBox().update(folder);
    }

    public void mergeItem(boolean needCreateId, MyAppItem toItem, MyAppItem fromItem, ArrayList<MyAppItem> items) {
        if (needCreateId) {
            // 创建一个ID
    ...
    public void deleteFolderItem(MyAppItem folder, ArrayList<MyAppItem> myAppItems, MyAppItem itemBeRemove) {
        ...
        getAppBox().update(itemBeRemove);
        getAppBox().update(myAppItems);
    }

    public void addItem(ArrayList<MyAppItem> myAppItems) {
        for (MyAppItem it : myAppItems) {
            it.setSort(myAppItems.indexOf(it));// 设置sort
            it.setDisplay(true);
        }
